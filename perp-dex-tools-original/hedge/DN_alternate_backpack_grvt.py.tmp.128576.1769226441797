...
class DNHedgeBot:
    ...
    async def reconcile_positions(self):
        ...
        - WebSocket is for real-time reference only
    ...
    async def place_hedge_order(
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # SOLUTION 3: CLOSE uses true market order for immediate fill
                if order_type == "CLOSE":
                    self.logger.info(
            ...
            for exchange_name, client in [
                ...
                try:
                    ...
                    if hasattr(client, "place_market_order"):
                        # Check if it's Backpack (uses 'direction') or GRVT (uses 'side')
                        if self.primary_exchange == "backpack" and exchange_name == "PRIMARY":
                            # Backpack uses 'direction' parameter
                            await client.place_market_order(
                                contract_id=client.config.contract_id,
                                quantity=close_qty,
                                direction=close_side,  # Backpack uses 'direction'
                            )
                        else:
                            # GRVT uses 'side' parameter
                            await client.place_market_order(
                                contract_id=client.config.contract_id,
                                quantity=close_qty,
                                side=close_side,
