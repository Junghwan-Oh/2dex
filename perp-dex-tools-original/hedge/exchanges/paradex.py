...
class ParadexClient(BaseExchangeClient):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._local_position = Decimal(0)  # Local position tracking for get_ws_position()
        self._last_sync_time = 0  # Last REST API position sync time
        self._sync_interval = 3  # Sync with REST API every 3 cycles (reduced from 5 for earlier drift detection)
        ...
        async def order_update_handler(ws_channel, message):
            ...
            if ws_channel == ParadexWebsocketChannel.ORDERS:
                ...
                if order_id and status:
                    ...
                    # Update local position tracking on fills
                    if mapped_status == 'FILLED':
                        fill_qty = Decimal(filled_size)
                        if side == "buy":
                            self._local_position += fill_qty
                        else:
                            self._local_position -= fill_qty
                        self.logger.log(
                            f"[WS_POSITION] Updated position: {self._local_position} ({side} {fill_qty})",
                            "INFO",
                        )
    ...
    async def place_post_only_order(self, contract_id: str, quantity: Decimal, price: Decimal,
        ...
        Args:
            contract_id: Market symbol (e.g., ETH-USD-PERP)
            quantity: Order size
            price: Limit price
            side: Order side - can be string ("buy"/"sell") or OrderSide enum
        """
        from paradex_py.common.order import Order, OrderType, OrderSide, OrderStatus

        # Convert side to OrderSide enum (handles both string and OrderSide enum)
        if isinstance(side, OrderSide):
            # Already an OrderSide enum, use directly
            order_side_enum = side
        elif isinstance(side, str):
            # String, convert to OrderSide enum
            if side.lower() == "buy":
                order_side_enum = OrderSide.Buy
            else:
                order_side_enum = OrderSide.Sell
        else:
            raise Exception(f"[POST_ONLY] Invalid side type: {type(side)}, value: {side}")

        # Create order using Paradex SDK
        order = Order(
            market=contract_id,
            order_type=OrderType.Limit,
            order_side=order_side_enum,  # Use OrderSide enum
    ...
    async def place_market_order(self, contract_id: str, quantity: Decimal, side: str) -> OrderResult:
        """Place a market order with Paradex for immediate fill (crosses spread).

        Market orders execute immediately against the best available price,
        functioning as taker orders that cross the spread.
        ...
        # Market orders have limit_price=0 and execute immediately
        order = Order(
            market=contract_id,
            order_type=OrderType.Market,
            order_side=order_side_enum,
            size=quantity.quantize(self.order_size_increment, rounding=ROUND_HALF_UP),
            limit_price=Decimal("0"),  # Market orders use 0 as limit price
        )

        self.logger.log(f"[MARKET] Placing {side.upper()} market order for {quantity}", "INFO")
        ...
        # Market orders fill immediately, but wait briefly for confirmation
        await asyncio.sleep(0.5)

        order_info = await self.get_order_info(order_id)
        if order_info is not None:
            # Handle both OrderResult and OrderInfo objects
            fill_price = getattr(order_info, 'avg_fill_price', None)
            if fill_price is None:
                fill_price = getattr(order_info, 'price', 'N/A')
            status = getattr(order_info, 'status', 'UNKNOWN')

            self.logger.log(
                f"[MARKET] Filled: {side.upper()} {quantity} @ {fill_price} (status: {status})",
                "INFO"
            )
            return order_info
        else:
            ...
            return OrderResult(
                ...
                size=quantity,
                price=Decimal("0"),
                status="FILLED"  # Market orders should fill immediately
            )
    ...
    async def place_open_order(self, contract_id: str, quantity: Decimal, direction: str) -> OrderResult:
        ...
        while True:
            ...
            # Pass direction as string (place_post_only_order expects string, not OrderSide enum)
    ...
    async def place_close_order(self, contract_id: str, quantity: Decimal, price: Decimal, side: str) -> OrderResult:
        ...
        while True:
            ...
            # Pass side as string (place_post_only_order expects string, not OrderSide enum)
            order_result = await self.place_post_only_order(contract_id, quantity, adjusted_price, side)
            order_status = order_result.status
            order_id = order_result.order_id
    ...
    async def get_account_positions(self) -> Decimal:
        ...
        for position in positions:
            if isinstance(position, dict) and position.get('market') == self.config.contract_id and position.get('status') == 'OPEN':
                # Warn about direction mismatch but don't raise error
                # This allows auto-cleanup logic to handle residual positions
                if position.get('side') == 'LONG' and self.config.direction == 'sell':
                    self.logger.log(
                        f"WARNING: Long position found with sell direction. Auto-cleanup should handle this.",
                        "WARNING"
                    )
                elif position.get('side') == 'SHORT' and self.config.direction == 'buy':
                    self.logger.log(
                        f"WARNING: Short position found with buy direction. Auto-cleanup should handle this.",
                        "WARNING"
                    )

                # Return position size (positive regardless of side)
