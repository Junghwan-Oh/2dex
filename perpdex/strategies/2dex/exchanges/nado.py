    ...
    async def place_ioc_order(self, contract_id: str, quantity: Decimal, direction: str) -> OrderResult:
        ...
        while retry_count < max_retries:
            try:
                ...
                # Build order parameters with IOC (Immediate-Or-Cancel)
                order = OrderParams(
                    sender=SubaccountParams(
                        subaccount_owner=self.owner,
                        subaccount_name=self.subaccount_name,
                    ...
                    nonce=gen_order_nonce(),
                    appendix=build_appendix(order_type=OrderType.IOC, isolated=False)  # Cross margin - requires less collateral than isolated
                )
