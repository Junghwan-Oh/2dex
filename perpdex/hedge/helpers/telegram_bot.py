...
class TelegramBot:
    def __init__(self, token: str, chat_id: str, base_url: Optional[str] = None):
        ...
        # Clear pending updates on initialization
    ...
    def clearPendingUpdates(self):
        ...
        try:
            ...
            if updates:
                ...
                maxUpdateId = max(update.get("update_id", 0) for update in updates)
                self.lastUpdateId = maxUpdateId
                print(f"Telegram: Cleared {len(updates)} pending updates (last ID: {maxUpdateId})")
            else:
                print("Telegram: No pending updates to clear")
        except Exception as e:
            print(f"Telegram: Error clearing pending updates: {e}")
    ...
    def sendMenu(self, text: str = "Select a command:", parse_mode: str = "HTML") -> Dict[str, Any]:
        """Send inline keyboard menu to Telegram"""
        inlineKeyboard = [
            [
    ...
    def answerCallbackQuery(self, callbackQueryId: str, text: str = "") -> Dict[str, Any]:
        """Answer callback query to remove loading state from button"""
        payload = {
            "callback_query_id": callbackQueryId,
            "text": text
        }
        return self._send_message("answerCallbackQuery", payload)
    ...
    def processUpdates(self) -> Optional[str]:
        """Process updates and return command if any action is needed.

        Returns:
            - 'stop' or 'kill' for stop commands
            - 'menu' for menu request
            - 'balance', 'position' for info commands
        ...
        for update in updates:
            ...
            if callbackQuery:
                ...
                fromChat = callbackQuery.get("message", {}).get("chat", {})
                chatId = str(fromChat.get("id", ""))

                # Only process from authorized chat
                if chatId == str(self.chat_id):
                    # Answer callback to remove loading state
                    self.answerCallbackQuery(callbackQueryId, f"Processing: {callbackData}")
                    return callbackData

            # Check for text message commands
            message = update.get("message", {})
            chatId = str(message.get("chat", {}).get("id", ""))
            ...
            if chatId == str(self.chat_id):
                ...
                if text in [cmd.lower() for cmd in self.stopCommands]:
                    return "stop"

                # Check for menu commands
                if text in [cmd.lower() for cmd in self.menuCommands]:
                    return "menu"
