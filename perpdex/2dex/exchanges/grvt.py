        ...
        while attempt < 15:
            ...
            if best_bid <= 0 or best_ask <= 0:
                return OrderResult(success=False, error_message='Invalid bid/ask prices')
                        ...
                        # Determine order side and price (passive maker - 1 tick from same side BBO)
            if direction == 'buy':
                # Place 1 tick below best bid (passive bid - more favorable price)
                order_price = best_bid - self.config.tick_size
            elif direction == 'sell':
                # Place 1 tick above best ask (passive ask - more favorable price)
            ...
            if attempt % 5 == 0:
                ...
                if current_close_orders - active_close_orders > 1:
                    ...
                    active_close_orders = current_close_orders
                        ...
                        # Adjust price for passive maker order
            best_bid, best_ask = await self.fetch_bbo_prices(contract_id)

            if side == 'sell':
                # Passive maker: place 1 tick above best ask
                adjusted_price = best_ask + self.config.tick_size
            elif side == 'buy':
                # Passive maker: place 1 tick below best bid
