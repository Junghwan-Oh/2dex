...
class DNHedgeBot:
    def __init__(
        ...
        self.log_filename = f"logs/dn_mean_reversion_grvt_primary_{self.primary_exchange}_{self.hedge_exchange}_{ticker}_log.txt"
        self.csv_filename = f"logs/dn_mean_reversion_grvt_primary_{self.primary_exchange}_{self.hedge_exchange}_{ticker}_trades.csv"
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if order_mode == "TAKER_FALLBACK" and hasattr(
                    ...
                    while time.time() - start_wait < 5:  # Wait up to 5 seconds for aggressive orders
                        ...
                        pos_change = max(abs(pos_ws - pos_before), abs(pos_api - pos_before))
                    ...
                    while time.time() - start_wait < maker_timeout:
                        ...
                        if not is_close:
                            ...
                            if side == "buy" and order_price > new_ask + self.hedge_tick_size:
                                self.logger.info(
                                    f"[{order_type}] BUY price {order_price} > ask {new_ask}, repricing..."
                                )
                                break
                            elif side == "sell" and order_price < new_bid - self.hedge_tick_size:
                                self.logger.info(
                ...
                if is_close:
                    ...
                    try:
                        ...
                        self.logger.warning(f"[{order_type}] REST API check failed: {e}, falling back to WS")
    ...
    async def trading_loop(self):
        ...
        for iteration in range(1, self.iterations + 1):
            ...
            while (
                ...
                success, primary_price, hedge_price = await self.execute_dn_cycle(direction)
