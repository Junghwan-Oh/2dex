...
### `/ultrapilot` vs `/swarm` 비교

| 특징 | Swarm | Ultrapilot |
|------|-------|------------|
| **목적** | 공유 작업 리스트에서 순차 처리 | 병렬 자율 실행 |
| **작업 분해** | 명시적 작업 정의 필요 | 자동 분해 |
| **사용자 개입** | 사용자가 작업 분할 명시 | 전체 자동화 |
| **에이전트 수** | 명시적 (N개 지정, 1-5개) | 자동 (3-5개) |
| **작업 단위** | 작은 청크 (개별 파일/기능) | 큰 단위 (전체 기능) |
| **충돌 해결** | 파일 소유권 추적 | 코디네이터 자동 관리 |
| **타임아웃** | 5분/작업, 자동 해제 | 자동 리소스 관리 |

**사용 시점**:

```bash
# Swarm: 명시적 작업 분할
/swarm 5:executor "fix TypeScript errors"
# → 5개 에이전트가 공유 리스트에서 각각 하나씩 작업 claim

# Ultrapilot: 전체 자동화
"ultrapilot: build me a REST API"
# → 계획 → 분해 → 3-5개 워커 동시 실행 → 자동 완료
```

**정리**:
- **Swarm**: 작업을 명시적으로 나누고, 여러 에이전트가 **순차적**으로 처리할 때 사용
- **Ultrapilot**: 사용자가 고수준 요청만 하면 **자동**으로 전체 작업을 병렬 처리할 때 사용
