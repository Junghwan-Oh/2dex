...
**Created:** 2026-01-26
**Revised:** 2026-01-26 (Iteration 2 - Addressed Critic Feedback)
**Priority:** CRITICAL (Emergency Unwind) + URGENT (GRVT API)
**Status:** REVISED (ITERATION 2) - Ready for Implementation
...
| Issue | Iteration 1 Approach | Iteration 2 (CORRECTED) |
|-------|---------------------|-------------------------|
| **Retry Logic** | Create NEW retry loop in grvt.py | MODIFY existing retry loop in DN_alternate_backpack_grvt.py (lines 802-1119) |
| **Session Refresh** | Implement immediately | DEFERRED - SDK API unclear, enhanced retry should suffice |
| **Test Commands** | Vague instructions | Concrete bash commands with grep patterns |
| **Root Cause** | Stated as fact | Acknowledged uncertainty, documented hypothesis |

### What Will Actually Be Done

**Priority 0 (CRITICAL) - Emergency Unwind Bug:**
- File: `DN_alternate_backpack_grvt.py` line 1682
- Change: `side=unwind_side` → `direction=unwind_side`
- Time: 5 minutes

**Priority 1 (URGENT) - Enhanced Retry Logic:**
- File: `DN_alternate_backpack_grvt.py` lines 802, 1110
- Change 1: `max_retries = 4` → `max_retries = 6`
- Change 2: `await asyncio.sleep(1)` → `await asyncio.sleep(2 ** (attempt - 1))`
- Time: 15 minutes

**Priority 2 (HIGH) - Circuit Breaker:**
- Add consecutive failure tracking to main trading loop
- Pause trading after 3 consecutive hedge failures
- Time: 20 minutes
