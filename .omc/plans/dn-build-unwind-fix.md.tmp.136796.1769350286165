...
1. **BUILD failure → Backpack position accumulation**: When BUILD succeeds on Backpack but fails on GRVT, the bot retries BUILD without unwinding Backpack, causing position accumulation
...
4. With max_iterations=3, total time budget = 3 iterations × ~0.7s = ~2.1s
...
### Deliverables
1. **BUILD failure → Backpack UNWIND logic**
...
### Definition of Done
- [ ] BUILD failures trigger Backpack-only unwind (using ACTUAL API position)
- [ ] NetDelta drift triggers alerts (NO auto-trading)
- [ ] GRVT failure root cause identified with evidence from ACTUAL logs
- [ ] Log analysis includes exact error messages, timing, and partial fill data
- [ ] All changes tested with 10+ iterations
- [ ] Zero position accumulation in test runs
- [ ] Binary search test includes cleanup attempt on pre-test check
...
- Unwind Backpack position using `get_account_positions()` API (ACTUAL position, not direction parameter)
- NetDelta drift monitoring in `reconcile_positions()` (READ-ONLY, no trading)
- Analyze EXISTING log files for root cause (grep/search patterns)
- Binary search test to find exact failure threshold
- Binary search test attempts cleanup before aborting on existing positions
- All changes maintain 1% NetDelta tolerance

### Must NOT Have
- Auto-trading logic for NetDelta drift (monitoring only)
- Unwind based on `direction` parameter instead of actual API position
...
- Abort binary search test without attempting cleanup first
...
```
┌─────────────────────────────────────────────────────────────┐
│ Phase 1: BUILD Failure Fix (HIGH PRIORITY)                  │
├─────────────────────────────────────────────────────────────┤
│ 1. Fix execute_dn_cycle() to check hedge result             │
│ 2. Add Backpack-only unwind when hedge fails                │
│ 3. Test with simulated GRVT failures                        │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│ Phase 2: NetDelta Auto-Recovery (MEDIUM PRIORITY)           │
├─────────────────────────────────────────────────────────────┤
│ 1. Add auto-unwind to reconcile_positions()                 │
│ 2. Configurable tolerance & cleanup threshold               │
│ 3. Test with induced drift                                  │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│ Phase 3: GRVT Root Cause Analysis (CRITICAL)                │
├─────────────────────────────────────────────────────────────┤
│ 1. Analyze existing iterative parameters vs defaults        │
│ 2. Trace execution flow for 0.5 ETH failure                 │
│ 3. Binary search test: 0.4, 0.45, 0.5, 0.55, 0.6 ETH       │
...
├─────────────────────────────────────────────────────────────┤
│ 1. Full trading_loop test (10+ iterations)                  │
│ 2. Verify zero position accumulation                        │
│ 3. Verify NetDelta stays within 1%                          │
└─────────────────────────────────────────────────────────────┘
```
...
### Task 1: BUILD Failure → Backpack UNWIND Logic (FIXED)
...
**CRITIC FEEDBACK ISSUE**: Previous plan used theoretical timing (0.1s + 0.5s + 0.1s) instead of analyzing actual log data. User explicitly said "iterative 그게 안되면 그게 왜 작동안됐는지를 찾아야지" (find WHY it doesn't work).
...
class GRVTThresholdTester:
    ...
    async def pre_test_safety_check_with_cleanup(self) -> bool:
        ...
        try:
            ...
            if abs(positions) > Decimal("0.001"):
                print(f"⚠️  WARNING: Existing position detected: {positions}")
                ...
                try:
                    ...
                    if abs(final_pos) < Decimal("0.001"):
                        print(f"✅ Cleanup successful - position closed")
                        return True
                    else:
                        print(f"❌ Cleanup FAILED - position still exists: {final_pos}")
    ...
    async def test_order_size(self, size: Decimal) -> dict:
        ...
        try:
            ...
            if buy_result['success']:
                print(f"✅ BUY filled: {result['buy_filled']} @ ${buy_result['average_price']:.2f}")
            else:
                print(f"❌ BUY failed: {buy_result.get('reason', 'unknown')}")
                ...
                if result['buy_filled'] > 0:
                    ...
                    if sell_result['success']:
                        ...
                        print(f"⚠️  Cleanup failed - manual intervention may be needed")
            ...
            if sell_result['success']:
                print(f"✅ SELL filled: {result['sell_filled']} @ ${sell_result['average_price']:.2f}")
            else:
                print(f"❌ SELL failed: {sell_result.get('reason', 'unknown')}")
    ...
    async def post_test_cleanup(self):
        ...
        try:
            positions = await self.client.get_account_positions()
            if abs(positions) > Decimal("0.001"):
                print(f"\n⚠️  POST-TEST CLEANUP: Closing remaining position: {positions}")
                side = "sell" if positions > 0 else "buy"
                await self.client.place_market_order(
                    contract_id=self.contract_id,
                    quantity=abs(positions),
                ...
                if abs(final_pos) < Decimal("0.001"):
                    ...
                    print(f"❌ Cleanup failed - position: {final_pos}")
        except Exception as e:
    ...
    def print_summary(self):
        ...
        for size, result in self.results.items():
            status = "✅" if result['buy_success'] and result['sell_success'] else "❌"
        ...
        if last_success and first_failure:
            print(f"✅ Last success: {last_success} ETH")
            print(f"❌ First failure: {first_failure} ETH")
            print(f"📍 Threshold: between {last_success} and {first_failure} ETH")
        elif last_success:
            print(f"✅ All tested sizes succeeded (up to {last_success} ETH)")
        elif first_failure:
            print(f"❌ All tested sizes failed (starting at {first_failure} ETH)")
...
### Functional Success
1. ✅ BUILD failures no longer accumulate Backpack positions
2. ✅ NetDelta drift triggers alerts with actionable information
3. ✅ GRVT failure root cause identified with evidence from ACTUAL logs
4. ✅ 0.2 ETH orders maintain 95%+ success rate
5. ✅ Clear logging of all recovery actions

### Technical Success
1. ✅ Fix applied to correct location (execute_dn_cycle hedge check)
2. ✅ Unwind uses ACTUAL API position via `get_account_positions()`, not `direction` parameter
3. ✅ Root cause analyzed from existing logs (Task 3 grep/search analysis)
4. ✅ Binary search test attempts cleanup before aborting
5. ✅ All changes tested with 10+ iterations
6. ✅ NetDelta monitoring is READ-ONLY (no auto-trading)
7. ✅ Zero position accumulation in test runs

### Operational Success
1. ✅ Logs clearly indicate what happened and why
2. ✅ Error messages actionable (not "Details not available")
3. ✅ Alerts provide position breakdown for manual intervention
4. ✅ Trading can continue safely after failures
...
- Should be at least: max_iterations × (0.5s sleep + 0.3s overhead) = 3 × 0.8 = 2.4s
...
**Order of execution**:
- Task 1 → prevents accumulation immediately
- Task 3 → identifies root cause (no code changes)
- Task 4 → confirms root cause with test data
- Analysis → determines parameter fix
- Task 5 → validates everything works

---

**Plan Status**: REVISED AFTER CRITIC FEEDBACK - READY FOR IMPLEMENTATION
**Estimated Time**: 4-6 hours (implementation + testing)
**Risk Level**: MEDIUM (read-only monitoring is safer than auto-trading)

**Key Changes from Previous Iteration**:
1. **Task 1 (Unwind Logic)**: Fixed to use `get_account_positions()` API instead of `direction` parameter
2. **Task 2 (NetDelta)**: Changed from auto-trading to READ-ONLY monitoring with alerts
3. **Task 3 (Log Analysis)**: Changed from theoretical calculations to ACTUAL log analysis with grep patterns
4. **Task 4 (Binary Search)**: Changed pre-test check to attempt cleanup before aborting

**Critic Feedback Addressed**:
- ✅ Issue 1: Task 1 now checks actual API position, not direction parameter
- ✅ Issue 2: Task 2 removed auto-trading, now read-only monitoring
- ✅ Issue 3: Task 3 specifies exact log files and search patterns to use
- ✅ Issue 4: Task 4 attempts cleanup via force_close before aborting
