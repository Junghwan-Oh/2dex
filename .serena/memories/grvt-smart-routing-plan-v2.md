# GRVT Smart Liquidity Routing - Implementation Plan V2
...
## RALPLAN V2 Corrections (After Critic Review)

### Critical Issues Fixed

| # | Issue | Fix Applied |
|---|-------|-------------|
| 1 | Missing helper functions | Added `extract_filled_quantity()` and `calculate_slippage_bps()` definitions |
| 2 | TradingLogger fix incomplete | Added exact wrapper method implementations |
| 3 | fetch_ticker() format unverified | Added verification step with actual SDK test |
| 4 | Order book format ambiguity | Specified GRVT uses dict format based on existing code |
| 5 | Timeout context missing | Added rationale for 10s/0.1 ETH formula |
...
**Current Problem**: Timeout = 30 + 30 * quantity seconds (45-60s for 1 ETH)
**User Feedback**: 45-60 seconds is too long for market microstructure-aware trading
**Fix**: Reduce to 5-20 seconds based on quantity
...
def calculate_timeout(quantity: Decimal) -> int:
    ...
    Previous: timeout = 30 + int(30 * float(quantity))  // 45-60s for 1 ETH
    New: timeout = max(5, min(20, 30 + int(10 * float(quantity))))
    """
    timeout = 30 + int(10 * float(quantity))
    return max(5, min(timeout, 20))
```
...
**File**: [exchanges/grvt.py](exchanges/grvt.py#L1)
**Location**: Add near top after imports
...
def calculate_slippage_bps(execution_price: Decimal, reference_price: Decimal) -> Decimal:
    ...
    return Decimal(str(slippage))  # Convert to Decimal
```
...
async def fetch_bbo(self, symbol: str) -> dict:
    ...
    Args:
        symbol: Trading pair (e.g., "ETH_USDT_Perp")
    ...
    Note: fetch_ticker() must be verified to return expected keys
    """
    ticker = await self.rest_client.fetch_ticker(symbol)
    
    # Verify keys exist
    if not all(key in ticker for key in ['best_bid_price', 'best_bid_size', 
...
async def find_hedge_price_with_liquidity(
    ...
    if slippage > max_slippage_bps:
        ...
        raise ValueError(
            ...
            f"available {available} at {max_slllipsage_bps} bps slippage "
...
- ✅ Timeout for 1 ETH is 20s or less
