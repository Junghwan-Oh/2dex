...
class GrvtClient(BaseExchangeClient):
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            if self._order_update_callback is not None:
                ...
                self.logger.log(
                    f"Deferred order subscription started for {self.config.contract_id}",
                    "INFO",
                )

            asyncio.create_task(self._subscribe_to_positions())
            self.logger.log("Position subscription started", "INFO")
    ...
    async def _subscribe_to_orders(self, callback):
        try:
            ...
            self.logger.log(f"Error in order subscription task: {e}", "ERROR")

    async def _subscribe_to_positions(self):
        try:

            async def position_callback(message: Dict[str, Any]):
                try:
                    self.logger.log(f"[POSITION WS] Raw message: {message}", "DEBUG")
                    ...
                    if "feed" in message:
                        ...
                        self.logger.log(
                            f"[POSITION WS] instrument={instrument}, data={data}",
                            "INFO",
                        )

                        if instrument == self.config.contract_id:
                            size = Decimal(data.get("size", "0"))
                            self._ws_position = size
                            self.logger.log(
                                f"[POSITION WS] Updated position: {size}", "INFO"
                            )
            ...
            await asyncio.sleep(0)
            self.logger.log("Successfully subscribed to position updates", "INFO")
        except Exception as e:
            self.logger.log(f"Error in position subscription task: {e}", "ERROR")
